...
class raceDataset(Dataset):
    def __init__(self, filename, sep_token, shuffle, percent=100):
        ...
        df['answersRightFormatting'] = df['answer'].map({"A": '0', "B": '1', "C": '2', "D": '3'})

        if shuffle:
            df = df.sample(frac=1).reset_index()

        if filename == 'race-train.json':
            max = round((len(df)*int(percent))/100)
            df = df.iloc[0:max]

        for j in range(len(df)):
            row = df.iloc[j]
            if "answersRightFormatting" in row:
                l = row["answersRightFormatting"]
            else:
                l = 0

            a1, a2, a3, a4 = row['options'][0], row['options'][1], row['options'][2], row['options'][3]
            c, q = row["article"], row["question"]
            ...
            content.append("Question: {} {} Answer: {} {} Context: {}".format(q, sep_token, a4, sep_token, c))
            if l == '0':
                labels += [1, 0, 0, 0]
            elif l == '1':
                labels += [0, 1, 0, 0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", default="microsoft/deberta-v3-base", help="Which model.")
    ...
    Path("results/race/").mkdir(parents=True, exist_ok=True)
    lf_name = "results/race/" + name.replace("/", "-") + ".txt"
    lf = open(lf_name, "a")
    lf.write(str(args) + "\n\n")
    lf.close()
